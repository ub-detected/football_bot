  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div>
          <h3 className="font-medium text-gray-800">{game.gameRoom.name}</h3>
          <p className="text-sm text-gray-500">{formatDate(game.playedAt || game.createdAt)}</p>
          <p className="text-sm text-gray-600 mt-1">
            <span className="font-medium">Локация:</span> {game.gameRoom.location}
          </p>
          {game.wasCaptain && (
...
const GameHistoryList: React.FC<GameHistoryListProps> = ({ history: initialHistory, loading: initialLoading }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Если пользователь прокрутил до конца списка (с запасом в 100px)
